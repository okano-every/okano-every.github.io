              ...
              return (
                <div key={k} style={{ ...S.row, borderLeft: `4px solid ${cfg.color}`, paddingLeft: 10, margin: "4px 0" }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "monospace", color: themeColors.text }}>{fmt(d.balance)}</div>
                    {d.credit > 0 && <div style={{ fontSize: 10, color: themeColors.amber, fontWeight: 600, marginTop: 2 }}>Credit +{fmt(d.credit)}</div>}
                    <div style={{ fontSize: 12, fontFamily: "monospace", color: pnlClr(d.closed_pl), fontWeight: 700, marginTop: 2 }}>{fmtPL(d.closed_pl)}</div>
                  </div>
                </div>
              );
            })}
      ...
      <div style={S.card}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", background: isDark ? "#1e2d45" : "#f1f5f9", borderRadius: 8, padding: 2, gap: 2 }}>
            ...
            <button style={{ padding: "5px 14px", borderRadius: 6, fontSize: 11, border: "none", cursor: "pointer", background: "#ffffff", color: "#0f172a", fontWeight: 600, boxShadow: "0 1px 3px rgba(0,0,0,.1)" }} onClick={() => setSumMode("year")}>年別</button>
          </div>
          <select style={{ fontSize: 12, padding: "6px 10px", borderRadius: 8, border: `1px solid ${themeColors.line}`, background: themeColors.card, color: themeColors.text, cursor: "pointer" }} value={sumYear} onChange={e => setSumYear(e.target.value)}>
...
function ChartTab({ metric, setMetric, themeColors, isDark, S }) {
  ...
  useEffect(() => {
    ...
    const METRIC_LABEL = { balance: "実質残高合計", equity: "Equity合計", closed_pl: "月次P/L合計" };
    const METRIC_COLOR = { balance: "#0052cc", equity: "#0284c7", closed_pl: "#7c3aed" };
    ...
    chartInst.current = new Chart(chartRef.current, {
      ...
      data: { 
        ...
        datasets: [{ 
          ...
          backgroundColor: METRIC_COLOR[metric] + "12", 
          borderWidth: 2.5, 
          pointRadius: 2, 
          tension: 0.2, 
          fill: true 
      ...
      options: {
        ...
        scales: {
          x: { ticks: { maxTicksLimit: 12, font: { size: 10 }, color: textClr }, grid: { display: false } },
          y: { ticks: { callback: v => (v/1000).toFixed(0)+"K", font: { size: 10 }, color: textClr }, grid: { color: gridClr } }
        }
      }
    });
...
function PLTab({ themeColors, isDark, S }) {
  ...
  useEffect(() => {
    ...
    chartInst.current = new Chart(chartRef.current, {
      type: "bar",
      data: { 
        labels: mths.map(m => m.slice(2)), 
        datasets: Object.entries(PL_GROUPS).map(([n, acs]) => ({
          label: n, 
          backgroundColor: PL_COLORS[n] + (isDark ? "bb" : "dd"), 
          borderColor: PL_COLORS[n], 
          borderWidth: 1,
          data: mths.map(ym => acs.reduce((s, k) => { 
            const d = RAW.find(r => r.ym===ym && `${r.broker}|${r.ac}`===k); 
            return s+(d?.closed_pl||0); 
          }, 0)),
        })) 
      },
      options: {
        ...
        plugins: { 
          legend: { display: true, position: "top", labels: { font: { size: 10 }, color: textClr, boxWidth: 10 } }, 
          tooltip: { callbacks: { label: c => ` ${c.dataset.label}: ${fmtPL(c.parsed.y)}円` } } 
        },
        scales: { 
          x: { stacked: true, ticks: { font:{size:10}, color: textClr }, grid:{display:false} }, 
          y: { stacked: true, ticks: { callback: v=>(v/1000).toFixed(0)+"K", font:{size:10}, color: textClr }, grid:{color: gridClr} } 
        }
      }
    });
    return () => { if (chartInst.current) { chartInst.current.destroy(); chartInst.current = null; } };
...
function TableTab({ themeColors, isDark, S }) {
  ...
  const pnlClr = (n) => (n > 0 ? themeColors.green : n < 0 ? themeColors.red : themeColors.muted);
  ...
  return (
    <div>
      <div style={S.frow}>
        ...
        <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color: themeColors.muted, cursor:"pointer", fontWeight: 500 }}>
          <input type="checkbox" checked={fz} onChange={e => setFz(e.target.checked)} style={{cursor:"pointer"}} /> 残高ゼロも表示
        </label>
      </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button
            ...
            style={{
              ...
              border: `1px solid ${themeColors.line}`, background: themeColors.bg,
              fontSize: 12, color: themeColors.muted, cursor: "pointer",
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: themeColors.text }}>FX 取引履歴ダッシュボード</div>
            <div style={{ fontSize: 11, color: themeColors.muted, marginTop: 2 }}>2023-06 〜 2026-05 ｜ {RAW.length} レコード</div>
          </div>
        </div>
        <div style={{ fontSize: 11, color: themeColors.muted, textAlign: "right", fontFamily: "monospace" }}>最終更新: 2026-06-28</div>
      </div>
      ...
      <div style={S.tabs}>
        {TABS.map(t => (
          <button 
            ...
            onClick={() => setTab(t.id)} 
            style={{
              background: tab===t.id ? themeColors.acc : "transparent",
              border: `1px solid ${tab===t.id ? themeColors.acc : "transparent"}`,
              color: tab===t.id ? "#fff" : themeColors.muted,
